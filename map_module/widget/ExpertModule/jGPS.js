var GPS = (function () {
    ...
    function mapGPSInit() {
        ...
        $.ajax({
            type: "GET",
            url: "https://olmap-28df7-default-rtdb.firebaseio.com/GPS_Station.json",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    $("#gps_station").append(new Option(data[i].Station));
                }
                GPSStationList = data;
            },
        });
        ...
        $("#GPS_county").change(function () {
            ...
            // 產生GPS測站列表
            console.log("fff");
            var data = GPSStationList.filter((st) => st.COUNTY === county);
        ...
        $("#gps_station").on("change", function () {
            ...
            $("#startdate").datepicker({
                dateFormat: "yy-mm-dd",
                minDate: new Date(
                    GPS.sdate.substring(0, 4),
                    String(parseInt(GPS.sdate.substring(4, 6)) - 1),
                    GPS.sdate.substring(6, 8)
                ),
                maxDate: new Date(
                    GPS.edate.substring(0, 4),
                    String(parseInt(GPS.edate.substring(4, 6)) - 1),
                    GPS.edate.substring(6, 8)
                ),
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data.forEach(function (value) {
                    var oneday = [
                        Date.UTC(
                            parseInt(value.year.substring(0, 4)),
                            parseInt(value.year.substring(4, 6)) - 1,
                            parseInt(value.year.substring(6, 8))
                        ),
                        parseFloat(parseFloat(value.hgt).toFixed(3)),
                    ];
                    ...
                    var onedayn = [
                        ...
                        parseFloat(parseFloat(value.dN).toFixed(3)),
                    ];
                    var onedaye = [
                        Date.UTC(
                            parseInt(value.year.substring(0, 4)),
                        ...
                        parseFloat(parseFloat(value.dE).toFixed(3)),
                    ];
